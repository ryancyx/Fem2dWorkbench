...
def _edge_id_by_points(bridge: WorkbenchBridge, start_point_id: str, end_point_id: str) -> str:
    for edge in _json_list(bridge.modelEdgesJson):
        if {edge["start_point_id"], edge["end_point_id"]} == {start_point_id, end_point_id}:
            return edge["id"]
    raise AssertionError(f"edge {start_point_id}-{end_point_id} not found")


def _build_shared_edge_rectangles(bridge: WorkbenchBridge) -> list[str]:
...
def test_stage18_shared_faces_material_mesh_workflow() -> None:
    pytest.importorskip("gmsh", reason="shared-face workflow requires gmsh backend")

    bridge = WorkbenchBridge()
    assert bridge.newProject(), bridge.statusText
    point_ids = _build_shared_edge_rectangles(bridge)
    ...
    p1, p2, p3, p4, p5, p6 = point_ids
    assert bridge.selectGeometryEdge(_edge_id_by_points(bridge, p4, p1)), bridge.statusText
    assert bridge.addConstraintToSelectedTarget(True, True), bridge.statusText
    assert bridge.selectGeometryEdge(_edge_id_by_points(bridge, p3, p6)), bridge.statusText
    ...
    if solve_ok:
        ...
        assert "材料" in bridge.statusText or "约束" in bridge.statusText or "奇异" in bridge.statusText
