...
class WorkflowOrchestrator:
    ...
    def _execute_solve(self, requested_result: str) -> None:
        ...
        self._validate_minimum_displacement_restraint()
    ...
    def _validate_minimum_displacement_restraint(self) -> None:
        """Reject a static model with no displacement restraint at all.

        The current solver can return a numerical result for this singular case instead
        of raising.  Keep this deliberately narrow: the Reviewer remains responsible for
        diagnosing the complete Tx/Ty/Rz restraint state and proposing the repair.
        """
        has_restraint = any(
            row.ux_fixed or row.uy_fixed
            for row in self.state.project.boundary_conditions
        )
        if not has_restraint:
            raise ValueError(
                "Static model has no displacement restraints; rigid-body Tx, Ty, and "
                "Rz modes are unconstrained"
            )
