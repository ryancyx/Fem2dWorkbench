...
that removes translations but leaves rigid-body rotation. Prefer one fully fixed non-zero
edge (for example the left edge with ux=0 and uy=0), which removes Tx, Ty, and Rz within
the current single-operation RepairProposal format.
