...
def test_stage18_contour_export(tmp_path: Path) -> None:
    pytest.importorskip("gmsh", reason="contour export workflow requires gmsh backend")
    ...
    export_dir = tmp_path / "contours"
    assert bridge.exportDisplacementContourData(str(export_dir)), bridge.statusText
    assert bridge.exportStressContourData(str(export_dir)), bridge.statusText

    displacement_file = export_dir / "displacement_contour_data.json"
    stress_file = export_dir / "stress_contour_data.json"
    assert displacement_file.exists()
    assert stress_file.exists()
    assert json.loads(displacement_file.read_text(encoding="utf-8"))["nodes"]
    assert json.loads(stress_file.read_text(encoding="utf-8"))["elements"]
